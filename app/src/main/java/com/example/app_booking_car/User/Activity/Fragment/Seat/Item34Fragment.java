...
public class Item34Fragment extends Fragment implements View.OnClickListener {

    private Button[] buttons;
    private Map<String, Boolean> selectedSeatsMap = new HashMap<>();
    private String carId;

    public Item34Fragment() {
    }

    public Item34Fragment(String carId) {
        this.carId = carId;
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_item34, container, false);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.buttonBuy) {
            ...
            handleSeatButtonClick((Button) v);
        }
    }

    private void handleSeatButtonClick(Button button) {
        String seatNumber = "Ghế " + button.getText().toString();
    ...
    private void retrieveCarDetails(String name, String email, String phoneNumber, String address) {
        ...
        String a =carId;
        // Query the car with the specified carId
        db.collection("cars")
                .document(a)
                .get()
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        // Car exists, extract the car data
                        Car car = documentSnapshot.toObject(Car.class);

                        // Display car details with carId in a notification or dialog
    ...
    private void saveBookingToFirestore(String id,String name, String email, String phoneNumber, String address, Car car, ArrayList<String> selectedSeats, int totalAmount) {
        ...
        newBooking.setSeatNumber(selectedSeats.toString()); // Lưu số ghế dưới dạng chuỗi
